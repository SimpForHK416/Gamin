...
// Map định nghĩa sự tiến hóa (Dùng để lọc Recruit)
...
data class GameState(
    ...
    // --- MỚI: Thêm map lưu máu của từng con trong team ---
...
class GameViewModel : ViewModel() {
    ...
    fun pickStarter(monsterName: String) {
        ...
        _gameState.value = _gameState.value.copy(
            ...
            teamHp = mapOf(monster.name to monster.hp), // Khởi tạo máu
            currentScreen = "MAP"
        )
    }
    ...
    fun onBattleWin(remainingHp: Int, nodeType: NodeType) {
        // Cập nhật máu sau trận
        val currentName = _gameState.value.playerMonster?.name ?: return
        val newTeamHp = _gameState.value.teamHp.toMutableMap()
        newTeamHp[currentName] = remainingHp

        if (nodeType == NodeType.BOSS) {
            _gameState.value = _gameState.value.copy(currentHp = remainingHp, teamHp = newTeamHp, lastBattledNodeType = nodeType, currentScreen = "MAJOR_UPGRADE")
            return
        }
        ...
        val statReward = Reward.StatUpgrade(randomStat, amount, "Tăng $amount $randomStat")
        val healReward = Reward.Heal(50, "Hồi phục 50% HP")
        val buffReward = dbHelper.getRandomOneBuff()?.let { Reward.SkillEffect(it) }
        val picks = if (nodeType == NodeType.ELITE) 2 else 1

        _gameState.value = _gameState.value.copy(currentHp = remainingHp, teamHp = newTeamHp, currentScreen = "BUFF_SELECT", optionStat = statReward, optionHeal = healReward, optionBuff = buffReward, rewardsToPick = picks, lastBattledNodeType = nodeType)
    }
    ...
    // Cập nhật máu khi quái bị chết trong trận
    ...
    fun applyReward(reward: Reward, targetMonster: Monster) {
        ...
        var optStat = currentState.optionStat
        var optHeal = currentState.optionHeal
        var optBuff = currentState.optionBuff
            ...
            if (m.name == targetMonster.name) {
                when(reward) {
                    is Reward.StatUpgrade -> {
                        optStat = null
                        val newM = when(reward.statName) {
                            "HP" -> m.copy(hp = m.hp + reward.value)
                            "Atk" -> m.copy(atk = m.atk + reward.value)
                            "Def" -> m.copy(def = m.def + reward.value)
                            "Speed" -> m.copy(speed = m.speed + reward.value)
                            else -> m
                        }
                    ...
                    is Reward.Heal -> {
                        optHeal = null
                        // Hồi máu trong map lưu trữ
                        val currentMonsterHp = newTeamHp[m.name] ?: m.hp
                        val healAmount = (m.hp * 0.5).toInt()
                        val newMonsterHp = (currentMonsterHp + healAmount).coerceAtMost(m.hp)
                        newTeamHp[m.name] = newMonsterHp

                        if (m.name == currentState.playerMonster?.name) hp = newMonsterHp
                        m
                    }
                    else -> m
                }
        ...
        val nextScreen = if (newPicks <= 0) {
            if (currentState.lastBattledNodeType == NodeType.BOSS) "MAJOR_UPGRADE" else "MAP"
        } else "BUFF_SELECT"
    ...
    fun onMajorUpgradeFinished(newTeam: List<Monster>) {
        val upgradedPlayer = newTeam.first()
        val newMap = GauntletMapGenerator.generateRandomMap()

        // Cập nhật lại HP map
        val newTeamHp = _gameState.value.teamHp.toMutableMap()
        // Hồi full máu cho cả team coi như thưởng
        newTeam.forEach { newTeamHp[it.name] = it.hp }

        _gameState.value = _gameState.value.copy(
            playerMonster = upgradedPlayer, team = newTeam, currentHp = upgradedPlayer.hp, teamHp = newTeamHp,
            mapData = newMap, currentNode = null, currentScreen = "MAP"
        )
    }
    ...
    fun getAvailableRecruits(): List<Monster> {
        val currentTeamNames = _gameState.value.team.map { it.name }
        val allStarters = dbHelper.getStartingMonsters()
        return allStarters.filter { starter ->
            val evoName = EVO_CHECK_MAP[starter.name]
            val isAlreadyInTeam = currentTeamNames.contains(starter.name)
            val isEvoInTeam = evoName != null && currentTeamNames.contains(evoName)
            !isAlreadyInTeam && !isEvoInTeam
        }
    }
    ...
    fun swapTeamMembers(index1: Int, index2: Int) {
        ...
        if (index1 in currentTeam.indices && index2 in currentTeam.indices) {
            ...
            val newActive = currentTeam[0]
            val newSkills = dbHelper.getSkillsForMonster(newActive.name)
            val newHp = _gameState.value.teamHp[newActive.name] ?: newActive.hp
    ...
    fun switchActiveMonsterInBattle(newMonster: Monster) {
        val newSkills = dbHelper.getSkillsForMonster(newMonster.name)
        val newHp = _gameState.value.teamHp[newMonster.name] ?: newMonster.hp
    ...
    // --- HÀM MỚI: Lấy danh sách quái còn sống ---
    fun getSurvivors(): List<Monster> {
        val teamHp = _gameState.value.teamHp
        return _gameState.value.team.filter { monster ->
            val hp = teamHp[monster.name] ?: monster.hp
            hp > 0
        }
    }
}